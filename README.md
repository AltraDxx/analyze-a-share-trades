...
三类决策使用同一个事件推演引擎：先识别市场、板块或个股正在经历的连续盘面事件，推断它最终想实现什么、现在处于什么阶段、当前必须完成什么，再选择最有区分力的数据验证并形成交易动作。市场、板块、公告和价量是检验推断的证据源，不是并列打分项或主力标签。

这里的“想做什么”不是声称读懂真实账户，而是找出最能贯通公开行为的事件目标。Skill 会向用户展示简洁的推断路径：事件从何种变化开始、为什么偏偏此时发生、哪些关键行为与普通波动不符、为何主判断优于竞争解释、下一步应出现什么以及何时失效。事件跨度由行为连续性决定，不会把当天涨跌当成完整故事，也不会每天重新编一个互不相干的解释。

正式答复会在行情指标和个股列表之前，先直接回答“事件要做什么、处于什么阶段、它现在要做什么”，再展示推断路径和验证证据；盘面当前动作不会被“用户应该等待”替代。
...
Agent 会自主选择最小分析范围：已经给出持仓或候选股票时不扫描全市场；只有未给候选而询问买什么，或明确要求全市场筛选时才生成候选池。参考资料也只在能够检验当前事件目标、阶段、当前动作或改变用户动作时按需读取。
